...
def read_one_image(name):
    images = []
    images.append(load_image('images/test/{0}.png'.format(name)))
    return images

def read_dataset(percentage=1.0, cutoff=0.7):
    labels = []
    with open('{0}/labels.txt'.format(data_path), 'r') as f:
        lines = f.readlines()
        for l in lines:
    ...
    images = []
    n = len(glob('{0}/tmp/img/*.png'.format(data_path)))
    assert n == len(labels)
    max_images = int(percentage * n)
    labels = labels[:max_images]
    print('found {} images.'.format(n))
    for i in range(1, n + 1):
        f = '{0}/tmp/img/img_{1}.png'.format(data_path, i)
...
def next_batch(arr, arr2, index, slice_size, debug=False):
    ...
    index *= batch_size
    total = len(arr)
    updated_index = index % total
    if updated_index + slice_size > total:
        updated_index = 0
        has_reset = True
    beg = updated_index
...
def restore(sess, saver):
    ...
    checkpoints = natsorted(glob('{0}/checkpoints/mnist-cluttered*'.format(data_path)), key=lambda y: y.lower())
    start_i = 0
    len_checkpoints = len(checkpoints)
    if len_checkpoints > 0:
...
def save(sess, saver, i):
    #mkdir_p('checkpoints')
    saver.save(sess, '{0}/checkpoints/mnist-cluttered'.format(data_path), global_step=i)
